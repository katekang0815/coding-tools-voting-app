...
export default function ToolsGrid() {
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async (toolId: number) => {
      ...
      // Determine the current like status and what the new status will be
      const currentLikes = previousLikes as any[] || [];
      const existingLike = currentLikes.find((like) => like.toolId === toolId);
      const wasLiked = existingLike?.liked || false;
      const willBeLiked = !wasLiked;
      const countIncrement = willBeLiked ? 1 : -1;

      // Optimistically update likes first
      queryClient.setQueryData(
        ["/api/tools/likes", actualUserId],
        (old: any[]) => {
          if (!old) return [{ toolId, liked: willBeLiked }];
          
          const existingIndex = old.findIndex((like) => like.toolId === toolId);
          if (existingIndex >= 0) {
            return old.map((like, index) =>
              index === existingIndex ? { ...like, liked: willBeLiked } : like
            );
          } else {
            return [...old, { toolId, liked: willBeLiked }];
          }
        },
      );

      // Then update tool counts based on the determined increment
      queryClient.setQueryData(["/api/tools"], (old: any[]) => {
        return (
          old?.map((tool) =>
            tool.id === toolId
              ? { ...tool, likeCount: Math.max(0, tool.likeCount + countIncrement) }
              : tool,
          ) || []
        );
      });
